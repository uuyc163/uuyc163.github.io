...
function card(key,h){var L=window.LABELS||{},F=window.FIELDS||[];
  ...
  return p+'</div>';}
function find(raw){var C=window.CODES||{}; raw=(raw||"").trim().toLowerCase().replace(/\s+/g,""); if(!raw)return null;
  for(var k in C){if(k.toLowerCase()===raw)return[k,C[k]];}
  for(var k2 in C){var hay=(k2+" "+(C[k2].alias||"")+" "+(C[k2].name||"")).toLowerCase().replace(/\s+/g,"");
...
function table(){var C=window.CODES||{},H=window.THEAD||[];
  var rows=Object.keys(C).map(function(k){var h=C[k];
    return '<tr><td class="code">'+esc(h.name||k)+'</td><td>'+esc(h.cat||"")+'</td><td>'+esc(h.sym||"-")+'</td><td>'+esc(h.fix||"-")+'</td></tr>';}).join("");
...
(function(){var chips=window.CHIPS||[],box=document.getElementById("chips");if(!box)return;
  chips.forEach(function(c){var b=document.createElement("span");b.className="chip";b.textContent=c;
   b.onclick=function(){document.getElementById("q").value=c;go();};box.appendChild(b);});})();
table();
